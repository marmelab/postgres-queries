...
describe('List', () => {
    ...
    describe('Functor law', () => {
        ...
        it('Functor identity law', () => {
            expect(List.of(5).map(identity)).toEqual(List.of(5));
        });
    });
    ...
    describe('Monad Law', () => {
        ...
        it('Monad Right identity law', () => {
            expect(List.of(5).chain(List.of)).toEqual(List.of(5));
        });

        it('Monad Left identity law', () => {
            expect(List.of(5).chain(doubleList)).toEqual(doubleList(5));
        });
    });
    ...
    describe('Applicative Functor Law', () => {
        ...
        it('Homomorphism', async () => {
            await expect(List.of(double).ap(List.of(5))).toEqual(
                List.of(double(5)),
            );
        });
    ...
    describe('Traversable Law', () => {
        ...
        it('Identity', async () => {
            await expect(
                List.of('value')
                    ...
                    .flatten().values,
            ).toEqual(Identity.of(List.of('value')).flatten().values);
        });

        it('Composition', async () => {
            const identityOfIdentityOfList = Identity.of(
                Writer.of(List.of(true)),
            );
            expect(
                identityOfIdentityOfList.map(Compose.of).sequence(Compose.of),
            ).toEqual(
                Compose.of(
                    identityOfIdentityOfList
                        .sequence(List.of)
                        .map(v => v.sequence(List.of)),
                ),
            );
        });
        ...
        it('Naturality', () => {
            ...
            expect(writerToList(listOfWriter.sequence(Writer.of))).toEqual(
    ...
    describe('sequence', () => {
        it('should convert List([Writer(value), ...]) to a Writer(List([value, ...])) while preserving all log', () => {
            const writerList = new List([
                new Writer(1, ['zero', 'one']),
                new Writer(2, ['two']),
                new Writer(3, ['three']),
            ]);
            expect(writerList.sequence(Writer.of)).toEqual(
                new Writer(new List([1, 2, 3]), [
                    'three',
                    'two',
                    'zero',
                    'one',
    ...
    describe('ap', () => {
        it('should apply list of values to list of functions', () => {
            ...
            expect(functionsList.ap(valuessList)).toEqual(
                new List([11, 12, 13, 2, 4, 6]),
            );
        });
    });
    ...
    describe('chain', () => {
        it('should map fn then flatten the result', () => {
            expect(
                new List(['hello', ' ', 'world']).chain(
                    word => new List(word.split('')),
                ),
            ).toEqual(
                new List([
                    'h',
